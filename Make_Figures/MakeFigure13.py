...
map_files = [map_file_Lavelanet,map_file_Lavelanet]
names = ['Lavelanet_alt1','Lavelanet_alt2']
...
def plot_map(map_gdf,ax,legend=True,cmap='Greys',edgecolor='black',linewidth=1.0):
    ...
    if legend:
        cax.invert_yaxis()
        cax.text(1.8, 0.92, 'Younger', horizontalalignment='left',verticalalignment='center',transform=cax.transAxes)
        cax.text(1.8, 0.13, 'Older', horizontalalignment='left',verticalalignment='center',transform=cax.transAxes)
...
#Save the figure.
fig.savefig('Fig13.pdf',bbox_inches='tight')
